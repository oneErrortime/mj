...
import { METRIC, Errors } from "./moleculer-compat.js";
 ...
 */
export default function WorkflowsMiddleware(mwOpts) {
    ...
    const middleware = {
        ...
        created(_broker) {
            ...
            broker.wf.run = async (workflowName, payload, opts) => {
                ...
                return Workflow.createJob(await broker.wf.getAdapter(), workflowName, payload, opts);
            };
            /**
            ...
            broker.wf.remove = async (workflowName, jobId) => {
                Workflow.checkWorkflowName(workflowName);
                if (!jobId) {
                    return Promise.reject(new Errors.MoleculerError("Job ID is required!", 400, "JOB_ID_REQUIRED"));
                }
            ...
            broker.wf.triggerSignal = async (signalName, key, payload) => {
                if (!signalName) {
                    return Promise.reject(new Errors.MoleculerError("Signal name is required!", 400, "SIGNAL_NAME_REQUIRED"));
                }
            ...
            if (_.isPlainObject(svc.schema[mwOpts.schemaProperty])) {
                ...
                for (const [name, def] of Object.entries(svc.schema[mwOpts.schemaProperty])) {
                    ...
                    if (_.isFunction(def)) {
                        ...
                        throw new Errors.ServiceSchemaError(`Invalid workflow definition in '${name}' workflow in '${svc.fullName}' service!`, svc.schema);
                    }
                    if (wf.enabled === false)
                        continue;
                    if (!_.isFunction(wf.handler)) {
                        throw new Errors.ServiceSchemaError(`Missing workflow handler on '${name}' workflow in '${svc.fullName}' service!`, svc.schema);
                    }
                    ...
                    if (wf.params && broker.validator) {
                        ...
                        wf.handler = async (ctx) => {
                            ...
                            if (res === true)
                                ...
                                throw new Errors.ValidationError("Parameters validation error!", null, res);
                            }
                        };
                    }
