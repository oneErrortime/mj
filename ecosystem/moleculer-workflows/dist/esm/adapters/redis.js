...
import { Serializers, Errors } from "../moleculer-compat.js";
import { Utils } from "../moleculer-compat.js";
import { promises as fs } from "node:fs";
import path from "node:path";
import BaseAdapter from "./base.js";
import { WorkflowError, WorkflowAlreadyLocked, WorkflowTimeoutError, WorkflowMaximumStalled } from "../errors.js";
import * as C from "../constants.js";
import { parseDuration, humanize } from "../utils.js";
...
export default class RedisAdapter extends BaseAdapter {
        ...
        return new Promise(resolve => {
            ...
            if (this.opts.cluster) {
                if (!this.opts.cluster.nodes || this.opts.cluster.nodes.length === 0) {
                    throw new Errors.BrokerOptionsError("No nodes defined for Redis cluster in Workflow adapter.", "ERR_NO_REDIS_CLUSTER_NODES");
                }
    ...
    async processJob(jobId) {
        ...
        try {
            ...
            this.log("debug", this.wf.name, jobId, `Job finished in ${humanize(duration)}.`, result);
    ...
    async lock(jobId) {
        ...
        if (!lockRes)
            throw new WorkflowError(`Job ${jobId} is already locked.`);
        ...
        if (opts.isCustomJobId) {
            ...
            if (exists) {
                if (this.mwOpts.jobIdCollision == "reject") {
                    throw new WorkflowError(`Job ID '${job.id}' already exists.`, 400, "JOB_ID_EXISTS");
                }
                ...
                for (const jobId of activeJobIds) {
                    try {
                        ...
                        if (job && job.startedAt > 0) {
                            const timeout = parseDuration(job.timeout != null ? job.timeout : this.wf.opts.timeout);
                            if (timeout > 0) {
                                if (now - job.startedAt > timeout) {
                                    this.log("debug", this.wf.name, jobId, `Job timed out (${humanize(timeout)}). Moving to failed queue.`);
                                    await this.moveToFailed(jobId, new WorkflowTimeoutError(this.wf.name, jobId, timeout));
                                    // Unlock manually
                                    await this.commandClient.del(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId));
                                    this.wf.removeRunningJob(jobId);
                                }
    ...
    async moveStalledJobsToWaiting(jobId) {
        ...
        if (this.wf.opts.maxStalledCount > 0 && stalledCounter > this.wf.opts.maxStalledCount) {
            ...
            await this.moveToFailed(jobId, new WorkflowMaximumStalled(jobId, this.wf.opts.maxStalledCount));
            return;
        }
    ...
    async maintenanceRemoveOldJobs(queueName, retention) {
        this.log("debug", this.wf.name, null, `Maintenance remove old ${queueName} jobs... (retention: ${humanize(retention)})`);
        try {
