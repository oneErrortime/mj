...
import { WorkflowError, WorkflowTaskMismatchError, WorkflowSignalTimeoutError } from "./errors.js";
import * as C from "./constants.js";
import { parseDuration, getCronNextTime } from "./utils.js";
...
export default class Workflow {
    ...
    createWorkflowContext(job, events) {
            ...
            try {
                ...
                const result = await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        if (opts?.timeout) {
                            let remaining = parseDuration(opts.timeout) -
                                (Date.now() - (event?.ts || startTime));
                            if (remaining <= 0) {
                                remaining = 1000;
                            }
                            setTimeout(() => {
                                reject(new WorkflowSignalTimeoutError(signalName, key, opts.timeout));
                            }, remaining);
                        }
                    })
                ]);
        ...
        ctx.wf.task = async (taskName, fn) => {
            ...
            if (!fn)
                throw new WorkflowError("Missing function to run.", 400, "MISSING_FUNCTION");
            const event = getCurrentTaskEvent();
            if (event)
    ...
    static checkWorkflowName(workflowName) {
        ...
        if (!re.test(workflowName)) {
            throw new WorkflowError(`Invalid workflow name '${workflowName}'. Only alphanumeric characters, underscore, dot and dash are allowed.`, 400, "INVALID_WORKFLOW_NAME", {
                workflowName
            });
        }
        return workflowName;
    ...
    static checkSignal(signalName, key) {
        ...
        if (!re.test(signalName)) {
            throw new WorkflowError(`Invalid signal name '${signalName}'. Only alphanumeric characters, underscore, dot and dash are allowed.`, 400, "INVALID_SIGNAL_NAME", {
                signalName
            });
        }
        if (key != null && !re.test(key)) {
            throw new WorkflowError(`Invalid signal key '${key}'. Only alphanumeric characters, underscore, dot and dash are allowed.`, 400, "INVALID_SIGNAL_KEY", {
                key
            });
        }
    }
    ...
    static async createJob(adapter, workflowName, payload, opts) {
        ...
        if (opts.repeat) {
            if (!opts.jobId) {
                throw new WorkflowError("Job ID is required for repeatable jobs", 400, "MISSING_REPEAT_JOB_ID");
            }
            ...
            if (opts.repeat.endDate) {
                ...
                if (endDate < Date.now()) {
                    throw new WorkflowError("Repeatable job is expired at " +
