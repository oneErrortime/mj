...
const moleculer_compat_ts_2 = require("../moleculer-compat.js");
 ...
 */
class RedisAdapter extends base_ts_1.default {
    isWorker;
    commandClient;
    blockerClient;
    subClient;
    ...
    constructor(opts) {
        ...
        super(opts);
        this.opts = lodash_1.default.defaultsDeep(this.opts, {
            serializer: "JSON",
            drainDelay: 5,
            redis: {
    ...
    init(wf, broker, logger, mwOpts) {
        ...
        /** @type {Serializer} */
        this.serializer = moleculer_compat_ts_1.Serializers.resolve(this.opts.serializer);
        ...
        return new Promise(resolve => {
            ...
            if (this.opts.cluster) {
                if (!this.opts.cluster.nodes || this.opts.cluster.nodes.length === 0) {
                    throw new moleculer_compat_ts_1.Errors.BrokerOptionsError("No nodes defined for Redis cluster in Workflow adapter.", "ERR_NO_REDIS_CLUSTER_NODES");
                }
                client = new ioredis_1.Cluster(this.opts.cluster.nodes, this.opts.cluster.clusterOptions);
            }
            else if (this.opts.url) {
                client = new ioredis_1.Redis(this.opts.url);
            }
            else {
                client = new ioredis_1.Redis(this.opts.redis);
            }
            client.on("ready", () => {
                this.connected = true;
                resolve(client);
            ...
            client.on("error", err => {
                ...
                this.log("info", this.wf?.name ?? "", null, `Redis adapter (${name}) error`, err.message);
            });
            client.on("end", () => {
                this.connected = false;
                this.log("info", this.wf?.name ?? "", null, `Redis adapter (${name}) disconnected.`);
            });
        });
    }
    ...
    async createSubscriptionClient() {
        ...
        this.subClient.on("messageBuffer", (channelBuf, message) => {
            ...
            else if (channel.startsWith(this.prefix) &&
    ...
    async runJobProcessor() {
        if (!this.running || this.blockerClient.blocked)
            return;
        ...
        try {
            ...
            const jobId = await this.blockerClient.brpoplpush(this.getKey(this.wf.name, C.QUEUE_WAITING), this.getKey(this.wf.name, C.QUEUE_ACTIVE), this.opts.drainDelay);
    ...
    async processJob(jobId) {
        ...
        try {
            ...
            await this.commandClient.hsetnx(this.getKey(this.wf.name, C.QUEUE_JOB, jobId), "startedAt", job.startedAt);
            ...
            this.log("debug", this.wf.name, jobId, `Job finished in ${(0, utils_ts_1.humanize)(duration)}.`, result);
            await this.moveToCompleted(job, result);
        }
        catch (err) {
            if (err instanceof errors_ts_1.WorkflowAlreadyLocked) {
    ...
    async lock(jobId) {
        // Set lock
        const lockRes = await this.commandClient.set(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId), this.broker.instanceID, "EX", this.mwOpts.lockExpiration, "NX");
        this.log("debug", this.wf.name, jobId, "Lock result", lockRes);
        if (!lockRes)
            throw new errors_ts_1.WorkflowError(`Job ${jobId} is already locked.`);
        const lockExtender = async () => {
            ...
            const finishedAt = await this.commandClient.hget(this.getKey(this.wf.name, C.QUEUE_JOB, jobId), "finishedAt");
            ...
            const lockRes = await this.commandClient.set(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId), this.broker.instanceID, "EX", this.mwOpts.lockExpiration, "XX");
            if (!lockRes) {
        ...
        return async () => {
            ...
            if (this.disconnecting || !this.connected)
                return;
            const unlockRes = await this.commandClient.del(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId));
    ...
    async getJob(workflowName, jobId, fields) {
        ...
        const exists = await this.commandClient.exists(this.getKey(workflowName, C.QUEUE_JOB, jobId));
        if (!exists)
            return null;
        let job;
        if (fields === true) {
            job = await this.commandClient.hgetallBuffer(this.getKey(workflowName, C.QUEUE_JOB, jobId));
        }
        else {
            const values = await this.commandClient.hmgetBuffer(this.getKey(workflowName, C.QUEUE_JOB, jobId), ...fields);
    ...
    async getJobEvents(workflowName, jobId) {
        const jobEvents = await this.commandClient.lrangeBuffer(this.getKey(workflowName, C.QUEUE_JOB_EVENTS, jobId), 0, -1);
    ...
    async addJobEvent(workflowName, jobId, event) {
        if (!this.connected || this.disconnecting)
            return;
        await this.commandClient.rpush(this.getKey(workflowName, C.QUEUE_JOB_EVENTS, jobId), this.serializer.serialize({
            ...event,
            ts: Date.now(),
    ...
    async moveToCompleted(job, result) {
        ...
        if (this.jobResultPromises.has(job.id)) {
            ...
            await this.commandClient.publish(this.getKey(this.wf.name, C.FINISHED), this.serializer.serialize({
                jobId: job.id,
                result
            }));
        }
        if (job.parent) {
            await workflow_ts_1.default.rescheduleJob(this, this.wf.name, job.parent);
        }
    }
    /**
    ...
    getBackoffTime(retryAttempts) {
        ...
        const delay = (0, utils_ts_1.parseDuration)(opts.delay) ?? 100;
        return Math.min(delay * Math.pow(opts.factor || 1, retryAttempts), opts.maxDelay ?? Number.POSITIVE_INFINITY);
    }
    /**
    ...
    async moveToFailed(job, err) {
        ...
        if (err?.retryable) {
            const retryFields = await this.commandClient.hmget(this.getKey(this.wf.name, C.QUEUE_JOB, job.id), "retries", "retryAttempts");
            const retries = (retryFields[0] != null
            ...
            if (retries > 0 && retryAttempts < retries) {
                const pipeline = this.commandClient.pipeline();
                pipeline.hincrby(this.getKey(this.wf.name, C.QUEUE_JOB, job.id), "retryAttempts", 1);
                const backoffTime = this.getBackoffTime(retryAttempts);
                this.log("debug", this.wf.name, job.id, `Retrying job (${retryAttempts} attempts of ${retries}) after ${backoffTime} ms...`);
                const promoteAt = Date.now() + backoffTime;
                pipeline.hset(this.getKey(this.wf.name, C.QUEUE_JOB, job.id), "promoteAt", promoteAt);
        ...
        if (this.wf.opts.removeOnFailed) {
            ...
            if (err != null) {
                ...
                fields.error = this.serializer.serialize(this.broker.errorRegenerator.extractPlainError(err));
            }
    ...
    async saveJobState(workflowName, jobId, state) {
        await this.commandClient.hset(this.getKey(workflowName, C.QUEUE_JOB, jobId), "state", this.serializer.serialize(state));
    ...
    async getState(workflowName, jobId) {
        const state = await this.commandClient.hgetBuffer(this.getKey(workflowName, C.QUEUE_JOB, jobId), "state");
    ...
    async triggerSignal(signalName, key, payload) {
        ...
        const exp = (0, utils_ts_1.parseDuration)(this.mwOpts.signalExpiration);
        if (exp != null && exp > 0) {
    ...
    async newJob(workflowName, job, opts) {
        ...
        if (opts.isCustomJobId) {
            ...
            const exists = await this.commandClient.exists(this.getKey(workflowName, C.QUEUE_JOB, job.id));
            if (exists) {
                if (this.mwOpts.jobIdCollision == "reject") {
                    throw new errors_ts_1.WorkflowError(`Job ID '${job.id}' already exists.`, 400, "JOB_ID_EXISTS");
                }
                    ...
                    if (foundJob.finishedAt) {
                        ...
                        await this.commandClient.hdel(this.getKey(workflowName, C.QUEUE_JOB, job.id), ...C.RERUN_REMOVABLE_FIELDS);
                    }
                    else {
                        isLoadedJob = true;
                        job = foundJob;
        ...
        if (!isLoadedJob) {
            ...
            await this.commandClient.hmset(this.getKey(workflowName, C.QUEUE_JOB, job.id), this.serializeJob(job));
            if (job.repeat) {
                // Repeatable job
                this.log("debug", workflowName, job.id, "Repeatable job created.", job);
                await workflow_ts_1.default.rescheduleJob(this, workflowName, job);
            }
            else if (job.delay) {
                // Delayed job
                this.log("debug", workflowName, job.id, `Delayed job created. Next run: ${new Date(job.promoteAt).toISOString()}`, job);
                await this.commandClient.zadd(this.getKey(workflowName, C.QUEUE_DELAYED), job.promoteAt, job.id);
                // Publish promoteAt time to all workers
        ...
        job.promise = async () => {
            ...
            if (!job2) {
                throw new errors_ts_1.WorkflowError("Job not found", 404, "JOB_NOT_FOUND", { jobId: job.id });
            }
    ...
    async sendDelayedJobPromoteAt(workflowName, jobId, promoteAt) {
        ...
        const head = await this.commandClient.zrange(this.getKey(workflowName, C.QUEUE_DELAYED), 0, 0);
        if (head && head[0] === jobId) {
            this.log("debug", workflowName, jobId, "Publish delayed job promote time", new Date(promoteAt).toISOString());
    ...
    async getNextDelayedJobTime() {
        const first = await this.commandClient?.zrange(this.getKey(this.wf.name, C.QUEUE_DELAYED), 0, 0, "WITHSCORES");
        if (first?.length > 0) {
    ...
    async newRepeatChildJob(workflowName, job) {
        job.repeatCounter = await this.commandClient.hincrby(this.getKey(workflowName, C.QUEUE_JOB, job.parent), "repeatCounter", 1);
    ...
    async deleteKeys(pattern) {
        if (this.commandClient instanceof ioredis_1.Cluster) {
    ...
    async deleteKeysOnCluster(pattern) {
        ...
        await Promise.all(this.commandClient
            .nodes("master")
    ...
    async deleteKeysOnNode(node, pattern) {
        return new Promise((resolve, reject) => {
            ...
            stream.on("error", err => {
                this.logger.error(`Error occured while deleting keys '${pattern}' on Redis node.`, err);
                reject(err);
            });
    ...
    async lockMaintenance(lockTime, lockName = C.QUEUE_MAINTENANCE_LOCK) {
        // Set lock
        const lockRes = await this.commandClient?.set(this.getKey(this.wf.name, lockName), this.broker.instanceID, "PX", lockTime / 2, "NX");
        return lockRes != null;
    }
    /**
    ...
    async maintenanceDelayedJobs() {
        ...
        try {
            const now = Date.now();
            const jobIds = await this.commandClient.zrangebyscore(this.getKey(this.wf.name, C.QUEUE_DELAYED), 0, now);
            if (jobIds.length > 0) {
                try {
                    const pipeline = this.commandClient.pipeline();
                    for (const jobId of jobIds) {
                        ...
                        if (job) {
                            this.log("debug", this.wf.name, jobId, "Moving delayed job to waiting queue.");
                    ...
                    this.log("error", this.wf.name, null, "Error while moving delayed jobs to waiting queue", err);
                }
            }
        }
        catch (err) {
    ...
    async maintenanceActiveJobs() {
        ...
        try {
            const now = Date.now();
            const activeJobIds = await this.commandClient.lrange(this.getKey(this.wf.name, C.QUEUE_ACTIVE), 0, -1);
                ...
                for (const jobId of activeJobIds) {
                    try {
                        ...
                        if (job && job.startedAt > 0) {
                            const timeout = (0, utils_ts_1.parseDuration)(job.timeout != null ? job.timeout : this.wf.opts.timeout);
                            if (timeout > 0) {
                                if (now - job.startedAt > timeout) {
                                    this.log("debug", this.wf.name, jobId, `Job timed out (${(0, utils_ts_1.humanize)(timeout)}). Moving to failed queue.`);
                                    await this.moveToFailed(jobId, new errors_ts_1.WorkflowTimeoutError(this.wf.name, jobId, timeout));
                                    // Unlock manually
                                    await this.commandClient.del(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId));
                                    this.wf.removeRunningJob(jobId);
                                }
                        ...
                        this.log("error", this.wf.name, jobId, "Error while timeout active job", err);
                    }
                }
            }
        }
    ...
    async maintenanceStalledJobs() {
        ...
        try {
            const stalledJobIds = await this.commandClient.smembers(this.getKey(this.wf.name, C.QUEUE_STALLED));
            if (stalledJobIds.length > 0) {
                for (const jobId of stalledJobIds) {
                    try {
                        // Check the job lock is exists
                        const exists = await this.commandClient.exists(this.getKey(this.wf.name, C.QUEUE_JOB_LOCK, jobId));
                        if (exists == 0) {
                            // Check the job is in the active queue
                            const removed = await this.commandClient.lrem(this.getKey(this.wf.name, C.QUEUE_ACTIVE), 1, jobId);
                            if (removed > 0) {
                        ...
                        await this.commandClient.srem(this.getKey(this.wf.name, C.QUEUE_STALLED), jobId);
                    }
                    catch (err) {
                        this.log("error", this.wf.name, jobId, "Error while processing stalled job", err);
                    }
                }
            }
            ...
            const activeJobIds = await this.commandClient.lrange(this.getKey(this.wf.name, C.QUEUE_ACTIVE), 0, -1);
            if (activeJobIds.length > 0) {
                await this.commandClient.sadd(this.getKey(this.wf.name, C.QUEUE_STALLED), activeJobIds);
            }
        }
        catch (err) {
    ...
    async moveStalledJobsToWaiting(jobId) {
        const stalledCounter = await this.commandClient.hincrby(this.getKey(this.wf.name, C.QUEUE_JOB, jobId), "stalledCounter", 1);
        ...
        if (this.wf.opts.maxStalledCount > 0 && stalledCounter > this.wf.opts.maxStalledCount) {
            this.log("debug", this.wf.name, jobId, "Job is reached the maximum stalled count. It's moved to failed.");
            await this.moveToFailed(jobId, new errors_ts_1.WorkflowMaximumStalled(jobId, this.wf.opts.maxStalledCount));
            return;
        }
    ...
    async maintenanceRemoveOldJobs(queueName, retention) {
        this.log("debug", this.wf.name, null, `Maintenance remove old ${queueName} jobs... (retention: ${(0, utils_ts_1.humanize)(retention)})`);
        try {
            const minDate = Date.now() - retention;
            const jobIds = await this.commandClient.zrangebyscore(this.getKey(this.wf.name, queueName), 0, minDate);
            if (jobIds.length > 0) {
                const jobKeys = jobIds.map(jobId => this.getKey(this.wf.name, C.QUEUE_JOB, jobId));
                const jobEventsKeys = jobIds.map(jobId => this.getKey(this.wf.name, C.QUEUE_JOB_EVENTS, jobId));
                const pipeline = this.commandClient.pipeline();
                pipeline.del(jobKeys);
                pipeline.del(jobEventsKeys);
                pipeline.zremrangebyscore(this.getKey(this.wf.name, queueName), 0, minDate);
                await pipeline.exec();
                this.log("debug", this.wf.name, null, `Removed ${jobIds.length} ${queueName} jobs:`, jobIds);
            }
        }
        catch (err) {
            this.log("error", this.wf.name, null, `Error while removing old ${queueName} jobs`, err);
        }
    }
    /**
    ...
    async listCompletedJobs(workflowName) {
        return this.formatZrangeResultToObject(await this.commandClient.zrange(this.getKey(workflowName, C.QUEUE_COMPLETED), 0, -1, "WITHSCORES"));
    }
    /**
    ...
    async listFailedJobs(workflowName) {
        return this.formatZrangeResultToObject(await this.commandClient.zrange(this.getKey(workflowName, C.QUEUE_FAILED), 0, -1, "WITHSCORES"));
    }
    /**
    ...
    async listDelayedJobs(workflowName) {
        return this.formatZrangeResultToObject(await this.commandClient.zrange(this.getKey(workflowName, C.QUEUE_DELAYED), 0, -1, "WITHSCORES"), "promoteAt");
    }
    /**
    ...
    async listActiveJobs(workflowName) {
        return (await this.commandClient.lrange(this.getKey(workflowName, C.QUEUE_ACTIVE), 0, -1)).map(jobId => {
            return { id: jobId };
        });
    }
    /**
    ...
    async listWaitingJobs(workflowName) {
        return (await this.commandClient.lrange(this.getKey(workflowName, C.QUEUE_WAITING), 0, -1)).map(jobId => {
            return { id: jobId };
        });
    }
    /**
    ...
    async dumpWorkflows(folder, wfNames) {
        moleculer_compat_ts_2.Utils.makeDirs(folder);
    ...
    async dumpWorkflow(workflowName, folder = ".") {
        ...
        const deserializeData = value => {
            if (lodash_1.default.isString(value) || Buffer.isBuffer(value)) {
                try {
        ...
        const fileName = node_path_1.default.join(folder, `dump-${workflowName}.json`);
        await node_fs_1.promises.writeFile(fileName, JSON.stringify(dump, null, 2), "utf8");
