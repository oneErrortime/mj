import type { ServiceBroker, Logger } from "../moleculer-compat.ts";
import type { Job, JobEvent, WorkflowsMiddlewareOptions, SignalWaitOptions, CreateJobOptions } from "../types.ts";
import type Workflow from "../workflow.ts";
export interface BaseDefaultOptions extends Record<string, unknown> {
}
export type ListJobResult = {
    id: string;
};
export type ListFinishedJobResult = {
    id: string;
    finishedAt: number;
};
export type ListDelayedJobResult = {
    id: string;
    promoteAt: number;
};
/**
...
export default abstract class BaseAdapter {
     ...
     */
    init(wf: any, broker: any, logger: any, mwOpts: any): void;
    /**
     ...
     * @param args
     */
    log(level: any, workflowName: any, jobId: any, msg: any, ...args: any[]): void;
    /**
     ...
     */
    abstract startJobProcessor(): any;
    /**
     * Stop the job processor for the given workflow.
     */
    abstract stopJobProcessor(): any;
    /**
     * Save state of a job.
     *
     ...
     */
    abstract waitForSignal<TSignalResult = unknown>(signalName: string, key?: string, opts?: SignalWaitOptions): Promise<TSignalResult>;
    /**
     ...
     */
    abstract getJob(workflowName: string, jobId: string, fields?: string[] | true): Promise<Job | null>;
    /**
     * Finish a parent job.
     *
     * @param workflowName
     ...
     */
    abstract addJobEvent(workflowName: string, jobId: string, event: Partial<JobEvent>): Promise<void>;
    /**
     ...
     */
    abstract dumpWorkflows(folder: string, wfNames: string[]): any;
    /**
     ...
     * @param folder - The folder to save the dump files.
     */
    abstract dumpWorkflow(workflowName: string, folder: string): any;
    /**
     ...
     * @param type
     */
    sendJobEvent(workflowName: string, jobId: string, type: any): void;
    /**
